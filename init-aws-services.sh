...
init_services() {
  $AWS_CMD \
    ...
    --name "pleo-antaeus-billing-invoices-api" \
    --http-method "POST" \
    --invocation-endpoint http://pleo-antaeus:7000/rest/v1/billing \
  ...
  $AWS_CMD \
    ...
    --schedule-expression "rate(1 minute)" # cron(0 9 1W * ? *) for production use the expression

  $AWS_CMD \
      events put-targets \
      --rule $EVENT_RULE_NAME \
      --targets "Id"="1","Arn"="arn:aws:events:$AWS_REGION:$AWS_ACCOUNT:api-destination/pleo-antaeus-billing-invoices-api"
